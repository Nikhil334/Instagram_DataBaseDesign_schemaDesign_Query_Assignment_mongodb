...
const requestforfriend = (req) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (status == 0) {
            ...
            friendsdata = new friends_schema_1.friend({
            ...
            const data = yield friends_schema_1.friend.findOneAndUpdate({ $and: [{ reciever_id: req.body.reciever_id }, { sender_id: req.user.user_id }] }, { status: setstatus });
            return data;
        }
        else {
            setstatus = requestStatus.Reject;
            const data = yield friends_schema_1.friend.findOneAndDelete({ reciever_id: req.body.reciever_id });
            return data;
        }
    }
    catch (err) {
