...
const registerUsers = (req) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const registerdata = new user_schema_1.Register({
...
const loginUsers = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const user = yield user_schema_1.Register.findOne({ email });
        if (!user) {
            return false;
        }
        else {
            ...
            if (!passmatch) {
                ...
                const token = jsonwebtoken_1.default.sign({ email: user.email, user_id: user._id, username: user.username }, process.env.secretKey, { expiresIn: '1h' });
                console.log(token);
                yield (0, user_sessioncontroller_1.maintain_session_control)(user, token);
                return true;
            }
        }
    }
...
const logoutservice = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = req.user;
        const isUser = yield user_schema_1.Register.find({ email: user.email });
        console.log(isUser);
        if (isUser) {
            ...
            if (isSession) {
                if (isSession[0].status) {
                    ...
                    yield (0, user_sessionredis_1.distroySession)(isUser[0]);
                    return true;
                }
                else {
                    return false;
